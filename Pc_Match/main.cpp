...
int main()
{
    ...
    // =======================   optimized icp   =======================
     OptimizedICPGN icp;
     //parameters
    icp.SetTargetCloud(cloud_target_ptr);
    icp.SetMaxIterations(50);
    icp.SetMaxCorrespondDistance(0.05);
    icp.SetTransformationEpsilon(1e-6);

    // 初始变换矩阵
    Eigen::Matrix4f initial_guess = T_predict;

    // 结果变量
    Eigen::Matrix4f result_pose;

    // 执行配准
    if (icp.Match(cloud_source_ptr, initial_guess, cloud_source_opti_transformed_ptr, result_pose))
    {
